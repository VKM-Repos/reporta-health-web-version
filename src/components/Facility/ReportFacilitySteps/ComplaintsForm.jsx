...
export default function FacilityInfoForm(props) {
  ...
  const [userDetails, setUserDetails] = useState(
    useUserCredentialsStore.getState().userDetails
  );
  const {
  ...
  const handleSetFormData = async (data) => {
    ...
    toast.promise(authInstanceAxios.post(`/report`, reportFacilityFormData), {
      pending: "Please wait...",
      success: {
        render() {
          setTimeout(() => {
        ...
        validate={(values) => {
          ...
          if (values.complaints_factor.length <= 0) {
            errors.complaints_factor =
              "One or more  Complaints box need to be checked";
          }
          return errors;
        }}
