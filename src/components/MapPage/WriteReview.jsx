import LoadingSpinner from "@components/LoadingSpinner/LoadingSpinner";
import { reviewFacility } from "@services/mutation/reviewFacility.service";
import React, { useState } from "react";

const WriteReview = ({ facility, onClose, onSuccess }) => {
  const [rating, setRating] = useState(0);
  const [body, setBody] = useState("");
  const [isAnonymous, setIsAnonymous] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [successMsg, setSuccessMsg] = useState(false);
  const [errors, setErrors] = useState({});

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!rating) {
      setErrors({ rating: "Your rating is required" });
      return;
    }
    setErrors({});
    setIsLoading(true);
    try {
      await reviewFacility({ facilityId: facility.id, rating, body, isAnonymous });
      setSuccessMsg(true);
      setTimeout(() => {
        setBody("");
        setRating(0);
        setIsAnonymous(false);
        onSuccess && onSuccess();
        onClose();
      }, 2000);
    } catch (err) {
      const msg = err?.response?.data?.detail || err?.response?.data?.[0] || "Something went wrong";
      setErrors({ submit: msg });
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className="relative">
      <form onSubmit={handleSubmit} className="relative">
        <div className="flex my-4 w-full items-center justify-center gap-1">
          {Array.from({ length: 5 }, (_, i) => (
            <svg
              onClick={() => setRating(i + 1)}
              key={i}
              className="aspect-square w-[10%] cursor-pointer"
              viewBox="0 0 15 15"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M8.98116 2.81658L10.0078 4.86992C10.1478 5.15575 10.5212 5.42992 10.8362 5.48242L12.697 5.79158C13.887 5.98992 14.167 6.85325 13.3095 7.70492L11.8628 9.15158C11.6178 9.39658 11.4837 9.86908 11.5595 10.2074L11.9737 11.9983C12.3003 13.4157 11.5478 13.9641 10.2937 13.2233L8.54949 12.1908C8.23449 12.0041 7.71533 12.0041 7.39449 12.1908L5.65033 13.2233C4.40199 13.9641 3.64366 13.4099 3.97033 11.9983L4.38449 10.2074C4.46033 9.86908 4.32616 9.39658 4.08116 9.15158L2.63449 7.70492C1.78283 6.85325 2.05699 5.98992 3.24699 5.79158L5.10783 5.48242C5.41699 5.42992 5.79033 5.15575 5.93033 4.86992L6.95699 2.81658C7.51699 1.70242 8.42699 1.70242 8.98116 2.81658Z"
                fill={i < rating ? "#242F9B" : "none"}
                stroke="#242F9B"
                strokeWidth="0.7"
              />
            </svg>
          ))}
        </div>
        <textarea
          className="w-full h-[7rem] p-2 border border-black/20 rounded-sm focus:outline-none"
          value={body}
          placeholder="Describe your experience (optional)"
          onChange={(e) => setBody(e.target.value)}
        />
        <label className="flex items-center gap-2 mt-2 text-sm text-black/70 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={isAnonymous}
            onChange={(e) => setIsAnonymous(e.target.checked)}
            className="w-4 h-4 accent-primary cursor-pointer"
          />
          Post this review anonymously
        </label>
        {successMsg && <p className="text-green-600 text-xs mt-1">Review submitted!</p>}
        {errors.rating && <p className="text-red-500 text-xs mt-1">{errors.rating}</p>}
        {errors.submit && <p className="text-red-500 text-xs mt-1">{errors.submit}</p>}
        <div className="w-full flex items-center space-x-2 justify-center">
          <button onClick={onClose} className="text-black/50 py-2 px-4 mt-4" type="button">
            Cancel
          </button>
          <button
            className="bg-primary text-center text-white py-2 px-4 mt-4 rounded-md"
            type="submit"
            disabled={isLoading}
          >
            {isLoading ? <LoadingSpinner text="submitting" /> : "Submit"}
          </button>
        </div>
      </form>
    </div>
  );
};

export default WriteReview;
